...
func newSaramaConfig(opt ConsumerOption) *sarama.Config {
	...
	cfg.Net.SASL.SCRAMClientGeneratorFunc = func() sarama.SCRAMClient {
		return &xdgScramConsumerClient{hash: scram.SHA512}
	}
...
type xdgScramConsumerClient struct {
	hash scram.HashGeneratorFcn
	*scram.Client
	*scram.ClientConversation
}

func (x *xdgScramConsumerClient) Begin(userName, password, authzID string) error {
...
func (x *xdgScramConsumerClient) Step(challenge string) (string, error) {
...
func (x *xdgScramConsumerClient) Done() bool {
	if x.ClientConversation == nil {
		return false
	}
