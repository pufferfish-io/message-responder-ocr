...
func NewKafkaProducer(opt Option) (*KafkaProducer, error) {
	...
	cfg.Net.SASL.SCRAMClientGeneratorFunc = func() sarama.SCRAMClient {
		return &xdgScramProducerClient{hash: scram.SHA512}
	}
...
type xdgScramProducerClient struct {
	hash scram.HashGeneratorFcn
	*scram.Client
	*scram.ClientConversation
}

func (x *xdgScramProducerClient) Begin(userName, password, authzID string) error {
...
func (x *xdgScramProducerClient) Step(challenge string) (string, error) {
...
func (x *xdgScramProducerClient) Done() bool {
	if x.ClientConversation == nil {
		return false
	}
